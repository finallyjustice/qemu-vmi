...
{
	struct kvm_desc_struct seg_desc, old_desc;
	u8 dpl, rpl;
...
setup_syscalls_segments(struct x86_emulate_ctxt *ctxt,
			struct kvm_desc_struct *cs, struct kvm_desc_struct *ss)
{
...
static int emulate_store_desc_ptr(struct x86_emulate_ctxt *ctxt,
				  void (*get)(struct x86_emulate_ctxt *ctxt,
					      struct kvm_desc_ptr *ptr))
{
	struct kvm_desc_ptr kvm_desc_ptr;

	if (ctxt->mode == X86EMUL_MODE_PROT64)
		ctxt->op_bytes = 8;
	get(ctxt, &kvm_desc_ptr);
	if (ctxt->op_bytes == 2) {
		ctxt->op_bytes = 4;
		kvm_desc_ptr.address &= 0x00ffffff;
	}
	/* Disable writeback. */
	ctxt->dst.type = OP_NONE;
	return segmented_write(ctxt, ctxt->dst.addr.mem,
			       &kvm_desc_ptr, 2 + ctxt->op_bytes);
}
...
static int em_lgdt(struct x86_emulate_ctxt *ctxt)
{
	...
	rc = read_descriptor(ctxt, ctxt->src.addr.mem,
			     &kvm_desc_ptr.size, &kvm_desc_ptr.address,
			     ctxt->op_bytes);
	if (rc != X86EMUL_CONTINUE)
		return rc;
	ctxt->ops->set_gdt(ctxt, &kvm_desc_ptr);
	/* Disable writeback. */
	ctxt->dst.type = OP_NONE;
	return X86EMUL_CONTINUE;
}
...
static int em_lidt(struct x86_emulate_ctxt *ctxt)
{
	...
	if (rc != X86EMUL_CONTINUE)
		return rc;
	ctxt->ops->set_idt(ctxt, &kvm_desc_ptr);
	/* Disable writeback. */
	ctxt->dst.type = OP_NONE;
	return X86EMUL_CONTINUE;
}
