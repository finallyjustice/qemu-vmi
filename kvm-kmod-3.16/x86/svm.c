...
static __init int svm_hardware_setup(void)
{
	...
	if (kvm_boot_cpu_has(X86_FEATURE_NX))
		kvm_enable_efer_bits(EFER_NX);

	if (kvm_boot_cpu_has(X86_FEATURE_FXSR_OPT))
		kvm_enable_efer_bits(EFER_FFXSR);

	if (kvm_boot_cpu_has(X86_FEATURE_TSCRATEMSR)) {
		u64 max;
	...
	if (!kvm_boot_cpu_has(X86_FEATURE_NPT))
		npt_enabled = false;
...
static u64 svm_compute_tsc_offset(struct kvm_vcpu *vcpu, u64 target_tsc)
{
	...
	tsc = svm_scale_tsc(vcpu, kvm_native_read_tsc());
...
static void init_vmcb(struct vcpu_svm *svm)
{
	...
	if (kvm_boot_cpu_has(X86_FEATURE_PAUSEFILTER)) {
...
static int dr_interception(struct vcpu_svm *svm)
{
	...
	if (!kvm_boot_cpu_has(X86_FEATURE_DECODEASSISTS))
		return emulate_on_interception(svm);
...
static int svm_get_msr(struct kvm_vcpu *vcpu, unsigned ecx, u64 *data)
{
	...
	case MSR_IA32_TSC: {
		*data = svm->vmcb->control.tsc_offset +
			svm_scale_tsc(vcpu, kvm_native_read_tsc());
...
static int svm_set_msr(struct kvm_vcpu *vcpu, struct msr_data *msr)
{
	...
	case MSR_IA32_SYSENTER_ESP:
		...
		break;
	case MSR_IA32_DEBUGCTLMSR:
		if (!kvm_boot_cpu_has(X86_FEATURE_LBRV)) {
...
static void svm_set_supported_cpuid(u32 func, struct kvm_cpuid_entry2 *entry)
{
	...
	case 0x8000000A:
		...
		if (kvm_boot_cpu_has(X86_FEATURE_NRIPS))
